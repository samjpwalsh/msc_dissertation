...
agent = DQNAgent(observation_dimensions, num_actions)
reward_list = []
step_counter = 0
...
for episode in range(EPISODES):
    state = env.reset()[0]
    state = np.reshape(state, [1, observation_dimensions])
    done = False
    episode_reward = 0
    while not done:
        action = agent.choose_action(state)
        next_state, reward, done, _, _ = env.step(action)
        next_state = np.reshape(next_state, [1, observation_dimensions])
